...
│   ├── sitemap.xml             # Sitemap index (references sub-sitemaps)
│   ├── sitemap-pages.xml       # Static pages sub-sitemap
...
│   │   ├── text.ts             # messagesList: article content (EN/RU/ZH/HI/ES)
...
│   │   ├── zh.po / zh.js       # Chinese translations
│   │   ├── hi.po / hi.js       # Hindi translations
│   │   └── es.po / es.js       # Spanish translations
...
> Note: `sitemap.xml` is a sitemap index that references `sitemap-pages.xml` (static pages) and `sitemap-{collection}-{locale}.xml` (one file per collection and locale with all token URLs). All sub-sitemaps are auto-generated by `scripts/generate-sitemap.mjs`. After adding new collections or messages, run `yarn sitemap` to regenerate. The script config must be kept in sync with `src/constants/collections.ts` and `src/constants/text.ts`.
...
6. **Add i18n keys** — run `yarn extract`, translate the new entries in `src/locales/{locale}.po` for all target locales (`ru`, `zh`, `hi`, `es`), then `yarn compile`.
...
- **Target locales:** `ru` (Russian), `zh` (Chinese), `hi` (Hindi), `es` (Spanish)
- **Fallback:** unknown locale → `en`
- **Config:** `lingui.config.js` scans `src/` for all locales
